...
/*
int main() {
    // Create a ByteStream with a capacity of 100 bytes
    ByteStream stream(100);

    // Write some data to the stream
    std::string data = "Hello, World!";
    size_t bytesWritten = stream.write(data);
    std::cout << "Bytes written: " << bytesWritten << std::endl;

    // Read data from the stream
    std::string output = stream.read(bytesWritten);
    std::cout << "Read data: " << output << std::endl;

    // Check if the stream has ended
    if (stream.input_ended()) {
        std::cout << "Stream input has ended." << std::endl;
    }
